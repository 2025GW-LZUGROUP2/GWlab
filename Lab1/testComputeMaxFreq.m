...
syms t;
A=10;
phi= 2*pi*( 3*t^3  +3*t^2 +10*t);
SigVec =A* sin(phi); % 相位信号
timeInterval = [0, 1];                   % 时间区间[0,1]
timeLength=timeInterval(2)-timeInterval(1);
maxFreq = ExactEstmFreqBW(SigVec,t, timeInterval);
...
delta = 1/fs;
timeVec = a:delta:b;
SigVec = double(subs(SigVec, t, timeVec));
...
fprintf("理论最大瞬时频率估计值为%.2f Hz(若两值接近，则说明正确)",subs((diff(phi,t))/(2*pi),1));
...
plot(posFreq,abs(fftVec_posFreq));
hold on ;
...
xline(maxFreq,'r--','LineWidth',1);
xline(NyqFreq,'b--','LineWidth',1);
grid on ;
length(posFreq)

length(abs(fftVec_posFreq))
