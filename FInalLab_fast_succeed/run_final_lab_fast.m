...
% finalLab（并行加速）：PSD预计算 + 批量FFT矢量化 + 多次PSO并行
...
% 为PSO定义适应度函数句柄：向量化的负GLRT加频域缓存，矢量化 + 频域缓存
...
% 统一预分配 outs（并行/串行都用）
% outs(1:nRuns) = struct('bestFitness',Inf,'bestLocation',[],'allBestFit',[]);
outs = cell(1, nRuns);
% 并行开关，% 根据可用性配置并行处理
usePar = par.UseParallel && license('test','Distrib_Computing_Toolbox');

% 进度条变量
completed = 0; lastPct = -1;
fprintf('运行进度：  0%%（0/%d）\n', nRuns);
...
if usePar
    ...
    % 提交 nRuns 个异步任务
    futures(1:nRuns) = parallel.FevalFuture;
    for r = 1:nRuns
        futures(r) = parfeval(@local_one_pso_run, 1, fitHandle, popSize, iters);
    end

    % 逐个取回任务结果并更新进度

    for k = 1:nRuns%并行分支
        [idx, out] = fetchNext(futures);  % idx 为完成的那个任务编号
        outs{idx} = out;                  % 存回对应位置
        % progress_step();                  % 更新百分比（逐行打印）
        [completed, lastPct] = progress_step(completed, lastPct, nRuns); 
    end
    fprintf('\n');
else
    for r = 1:nRuns%串行分支
...
for r = 1:nRuns
    ...
    if out.bestFitness < bestFit
        bestFit     = out.bestFitness;
        bestLoc     = out.bestLocation;
        bestAllBest = out.allBestFit;
    end
end
...
E   = local_energy_fft(s, pre);         % (s|s)% 信号能量
su  = s / sqrt(E);                      % 单位SNR模板
llr = local_llr_fft(su, pre);           % (x|su)% 对数似然比
SNR = abs(llr);% 信噪比
...
%  保存结果到 /figure 文件夹（相对于本脚本）
scriptDir = fileparts(mfilename('fullpath'));
figDir = fullfile(scriptDir, 'figure');
if ~exist(figDir, 'dir'), mkdir(figDir); end

%  保存两张图（PNG），优先用 exportgraphics，老版本回退到 saveas
tFitPath = fullfile(figDir, 'time_fit.png');
convPath = fullfile(figDir, 'pso_convergence.png');
try
    exportgraphics(fig1, tFitPath, 'Resolution', 300);
    exportgraphics(fig2, convPath, 'Resolution', 300);
catch
    saveas(fig1, tFitPath);
    saveas(fig2, convPath);
end

%  保存文本结果（含 参数、耗时、完整 results 结构体）
txtPath = fullfile(figDir, 'final_result.txt');
fid = fopen(txtPath, 'w');
if fid ~= -1
    fprintf(fid, '最优参数：a1=%.6f, a2=%.6f, a3=%.6f,  SNR=%.6f\n', a1,a2,a3,SNR);
    fprintf(fid, '总耗时：%.3f 秒（约 %.2f 分钟）\n\n', elapsed, elapsed/60);
    resStr = evalc('disp(results)');     % 把结构体的显示内容抓出来
    fprintf(fid, '%s\n', strtrim(resStr));
    fclose(fid);
else
...
% 嵌套函数：进度显示（兼容 parfor/for）
function [completed, lastPct] = progress_step(completed, lastPct, nRuns)
    completed = completed + 1;
    pct = floor(100*completed/nRuns);
    if pct ~= lastPct
        fprintf('运行进度：%3d%%（%d/%d）\n', pct, completed, nRuns);
        lastPct = pct;
    end
...
% 批量 (s|s)% 同时计算所有信号的PSD加权能量
E  = pre.c * sum( (abs(S).^2) .* pre.invPSD, 2 );    % M×1% M×1能量向量

% 批量 (x|s) 同时计算所有信号的PSD加权内积
...
fval  = -GLRT;    % 返回负GLRT用于最小   PSO最小化 -GLRT
end
